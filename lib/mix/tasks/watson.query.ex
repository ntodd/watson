defmodule Mix.Tasks.Watson.Query do
  @moduledoc """
  Query the watson index.

  ## Usage

      mix watson.query TYPE [OPTIONS]
  ...
  ## Examples
      ...
      mix watson.query routes
      mix watson.query def --mfa MyApp.Accounts.get_user/1
      mix watson.query refs --mfa MyApp.Accounts.get_user/1
      mix watson.query callers --mfa MyApp.Accounts.get_user/1 --depth 2
      mix watson.query schema --module MyApp.Accounts.User
      mix watson.query impact --files lib/my_app/accounts.ex
  ...
  def run(args) do
    case args do
      [] ->
        Mix.shell().error("Usage: mix watson.query TYPE [OPTIONS]")
        Mix.shell().info("Run `mix help watson.query` for more information")
        exit({:shutdown, 1})
  ...
  defp run_query(query_type, args) do
    ...
    case Watson.Query.execute(String.to_atom(query_type), query_args, project_root: project_path) do
      {:ok, result} ->
